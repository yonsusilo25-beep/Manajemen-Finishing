...
<div class="page-heading">
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Edit Job</h3>
                <p class="text-subtitle text-muted">Update job and bill of materials</p>
            </div>
    ...
    <section class="section">
        <form action="{{ route('jobs.update', $job) }}" method="POST" id="job-form">
            @csrf
            @method('PUT')
                ...
                <div class="col-12">
                    ...
                    <div class="card mb-4">
                        <div class="card-header">
                            <h4 class="card-title">Edit Job: {{ $job->job_number }}</h4>
                        </div>
                        <div class="card-body">
                            @if(session('error'))
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('job_number') is-invalid @enderror"
                                           id="job_number" name="job_number" value="{{ old('job_number', $job->job_number) }}" required>
                                    @error('job_number')
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control @error('product_name') is-invalid @enderror"
                                           id="product_name" name="product_name" value="{{ old('product_name', $job->product_name) }}" required>
                                    @error('product_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" class="form-control @error('quantity') is-invalid @enderror"
                                           id="quantity" name="quantity" value="{{ old('quantity', $job->quantity) }}" min="1" required>
                                    @error('quantity')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select class="form-select @error('status') is-invalid @enderror"
                                        ...
                                        <option value="planned" {{ old('status', $job->status) == 'planned' ? 'selected' : '' }}>Planned</option>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="date" class="form-control @error('start_date') is-invalid @enderror"
                                           id="start_date" name="start_date" value="{{ old('start_date', $job->start_date->format('Y-m-d')) }}" required>
                                    @error('start_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="date" class="form-control @error('due_date') is-invalid @enderror"
                                           id="due_date" name="due_date" value="{{ old('due_date', $job->due_date->format('Y-m-d')) }}" required>
                                    @error('due_date')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('notes') is-invalid @enderror"
                                          id="notes" name="notes" rows="3">{{ old('notes', $job->notes) }}</textarea>
                                @error('notes')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div id="bom-container">
                                @php
                                    $boms = old('boms') ? collect(old('boms'))->map(function($bom, $index) {
                                        return (object)$bom;
                                    }) : $job->boms;
                                @endphp
                                    ...
                                    <div class="bom-item" data-index="{{ $index }}">
                                                ...
                                                <select class="form-select material-select @error('boms.'.$index.'.material_id') is-invalid @enderror"
                                                    ...
                                                    @foreach($materials as $material)
                                                        <option value="{{ $material->id }}"
                                                                ...
                                                                {{ (old('boms.'.$index.'.material_id', $bom->material_id ?? '') == $material->id) ? 'selected' : '' }}>
                                                ...
                                                <select class="form-select @error('boms.'.$index.'.department_id') is-invalid @enderror"
                                                    ...
                                                    @foreach($departments as $department)
                                                        <option value="{{ $department->id }}"
                                                                {{ (old('boms.'.$index.'.department_id', $bom->department_id ?? '') == $department->id) ? 'selected' : '' }}>
                                                            {{ $department->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <input type="number" step="0.01"
                                                       ...
                                                       value="{{ old('boms.'.$index.'.quantity_required', $bom->quantity_required ?? '') }}"
                                                       min="0.01" required>
                                            ...
                                            <div class="col-md-6 mb-3">
                                                ...
                                                <input type="text"
                                                       ...
                                                       value="{{ old('boms.'.$index.'.unit', $bom->unit ?? '') }}"
                                                       readonly required>
                                        ...
                                        <div class="row">
                                            <div class="col-12 mb-3">
                                                ...
                                                <textarea class="form-control" name="boms[{{ $index }}][notes]" rows="2">{{ old('boms.'.$index.'.notes', $bom->notes ?? '') }}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                @empty
